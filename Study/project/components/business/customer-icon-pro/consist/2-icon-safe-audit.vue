<template>
    <div class="customer-icon-item">
        <ui-tooltip2
            placement="top"
            appendToBody
            effect="light"
            popperClass="popper-customer-icon-simple"
            @on-show="onShow"
            @on-hide="onHide('icon-customer-safe-audit')">
            <span class="icon-customer-safe-audit icon-customer-trigger"></span>
            <template slot="content">
                <div class="customer-icon-content">
                    <p class="desc">此客户正在进行安全审核</p>
                </div>
            </template>
        </ui-tooltip2>
    </div>
</template>

<script>
import trackEventMixins from './track-event-mixins';
export default {
    name: 'customer-icon-safe-audit',
    mixins: [trackEventMixins],
    data () {
        return {};
    },
    props: {},
};
</script>